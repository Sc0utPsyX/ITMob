...
angular.module('app', []).controller('eventsController', function ($scope, $http) {
    ...
    $scope.loadEventTypeIsDefault = function () {
        $http.get(contextPath + 'api/v1/event_types/1').then(function (response) {
        $scope.eventTypeIsDefault = response.data;
            //console.log(response.data);
        });
    }
