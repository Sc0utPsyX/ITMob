angular.module('socialnetwork').controller('profileController', function ($rootScope, $scope, $http, $location, $localStorage) {

    const contextPath = 'http://localhost:8907/event/';
    const contextPathUser = 'http://localhost:9001/';
    ...
    $scope.findFollowers = function (event) {
        $http({
            url: contextPath + 'followers_event_members',
            method: 'GET',
            params: {
                "event_id": event.id
            }
    ...
    $scope.isAuthor = function (event) {
        return (event.author == $localStorage.itMobUser.username);
    };
